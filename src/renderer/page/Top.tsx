...
import { Container, DropdownProps, PaginationProps, Select } from "semantic-ui-react";
...
const sliceItems = (items: ItemForRenderer[], currentPage: number): ItemForRenderer[] => items.slice(currentPage * PAGE_SIZE, (currentPage + 1) * PAGE_SIZE);

const Top = (): JSX.Element => {
    const query = useQuery();
    const [state, setState] = useState(
        { items: [], totalPages: 0, currentPage: 0 } as { items: ItemForRenderer[], totalPages: number, currentPage: number }
    );
    ...
    const onPageChange = (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>, data: PaginationProps): void => {
        const value = event.currentTarget.innerText;
        let nextPage = -1;
        switch (value) {
            case '⟩':
                nextPage = Math.min(state.currentPage + 1, state.totalPages - 1);
                break
            case '⟨':
