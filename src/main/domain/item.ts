...
export const filterItems = (
    ...
    return items
        .filter(item =>
            parsedQuery["complete"]["title"].every(title => item.name === title)
        )
        .filter(item =>
            parsedQuery["complete"]["tag"].every(tag => item.tags.includes(tag))
        )
        .filter(item =>
            parsedQuery["part"]["title"].every(title =>
                item.name.includes(title)
            )
        )
        .filter(item =>
            parsedQuery["part"]["tag"].every(tag =>
                item.tags.some(attachedTag => attachedTag.includes(tag))
            )
        );
};
...
export const parseQuery = (query: string): ParsedQuery => {
    ...
    for (const c of `${query} `) {
        if (c === '"') {
            ...
            result[enclose === 2 ? "complete" : "part"][
                isTag ? "tag" : "title"
            ].push(word);
            word = "";
            enclose = 0;
...
export const getItemForRendererWithGroupdedTags = (
    id: string
): ItemForRendererWithGroupedTags => {
